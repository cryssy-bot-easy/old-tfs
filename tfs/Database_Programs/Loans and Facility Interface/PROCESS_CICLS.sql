...
P1: BEGIN
    ...
    DECLARE V_OTHERS_PROD_ID INTEGER;
    
    SET V_OTHERS_PROD_ID = ( SELECT PROD_ID FROM TFSDB2S.PROD_PAYMNT_CICLS_MAPPING
                             WHERE MOP_PRODUCT = 'OTHERS');
    ...
    WITH LATEST_TS AS (SELECT TRADEPRODUCTNUMBER, MAX(CREATEDDATE) CREATEDDATE
        FROM TRADESERVICE
        WHERE DOCUMENTCLASS IN ('DA','OA','LC','INDEMNITY') AND SERVICETYPE NOT IN ('UA_LOAN_SETTLEMENT')
        AND STATUS NOT IN ( 'EXPIRED', 'RETURNED', 'PENDING', 'MARV')
    ...
    SELECT SYSCOD, CIFNO, ACCTNO, NAME, CIFID, CIFCOD, ORGLMT, TOSBAL, PRODUCT, PRODSC
        ...
        FROM (
            ...
            , A.LOCSBL, CASE WHEN D.PROD_ID IS NULL THEN V_OTHERS_PROD_ID ELSE D.PROD_ID END AS PROD_ID, NOW() AS "CREATED_DATE"
        ...
        AND C.STATUS NOT IN ( 'EXPIRED', 'RETURNED', 'PENDING', 'MARV')
                UNION ALL
        SELECT 'TF', C.CIFNUMBER, REPLACE(A.DOCUMENTNUMBER, '-', ''), G.CFSNME, NULL, NULL, C.AMOUNT, A.OUTSTANDINGBALANCE, '25', NULL
            ...
            , C.CURRENCY, 'A', 0, 'CURRENT', '', 0, 0, CASE WHEN F.PROD_ID IS NULL THEN V_OTHERS_PROD_ID ELSE F.PROD_ID END AS PROD_ID, NOW() AS "CREATED_DATE"
        ...
        WHERE C.STATUS <> 'EXPIRED'
            ...
            AND E.STATUS NOT IN ( 'EXPIRED', 'RETURNED', 'PENDING', 'MARV')
                UNION ALL
        SELECT 'TF', A.CIFNUMBER, REPLACE(A.DOCUMENTNUMBER, '-', ''), G.CFSNME, NULL, NULL, DECODE(A.PRODUCTTYPE, 'DA', B.AMOUNT, C.AMOUNT)
            ...
            , A.CURRENCY, 'A', 0, 'CURRENT', '', 0, 0, CASE WHEN F.PROD_ID IS NULL THEN V_OTHERS_PROD_ID ELSE F.PROD_ID END AS PROD_ID, NOW() AS "CREATED_DATE"
            , DECODE(A.CURRENCY , 'PHP', DECODE(A.PRODUCTTYPE, 'DA', B.AMOUNT, C.AMOUNT)
                , DECODE(A.PRODUCTTYPE, 'DA', B.AMOUNT, C.AMOUNT) * (SELECT JFXDCR FROM IRR_RATES WHERE JFXDCD = A.CURRENCY))
            , DECODE(A.CURRENCY , 'PHP', DECODE(A.PRODUCTTYPE, 'DA', B.OUTSTANDINGAMOUNT, C.OUTSTANDINGAMOUNT)
                , DECODE(A.PRODUCTTYPE, 'DA', B.OUTSTANDINGAMOUNT, C.OUTSTANDINGAMOUNT) * (SELECT JFXDCR FROM IRR_RATES WHERE JFXDCD = A.CURRENCY))
        ...
        SELECT 'TF', B.CIFNUMBER, REPLACE(A.INDEMNITYNUMBER, '-', ''), F.CFSNME, NULL, NULL, A.SHIPMENTAMOUNT
            ...
            , DECODE(B.CURRENCY , 'PHP', A.SHIPMENTAMOUNT
                , A.SHIPMENTAMOUNT * (SELECT JFXDCR FROM IRR_RATES WHERE JFXDCD = B.CURRENCY))
            , B.CURRENCY, 'A', 0, 'CURRENT', '', 0, 0, CASE WHEN E.PROD_ID IS NULL THEN V_OTHERS_PROD_ID ELSE E.PROD_ID END AS PROD_ID, NOW() AS "CREATED_DATE"
            , DECODE(B.CURRENCY , 'PHP', A.SHIPMENTAMOUNT
                , A.SHIPMENTAMOUNT * (SELECT JFXDCR FROM IRR_RATES WHERE JFXDCD = B.CURRENCY))
            , DECODE(B.CURRENCY , 'PHP', A.SHIPMENTAMOUNT
                , A.SHIPMENTAMOUNT * (SELECT JFXDCR FROM IRR_RATES WHERE JFXDCD = B.CURRENCY))
        ...
        INNER JOIN LATEST_TS C
            ON A.REFERENCENUMBER = C.TRADEPRODUCTNUMBER
        INNER JOIN TRADESERVICE D
            ON C.TRADEPRODUCTNUMBER = D.TRADEPRODUCTNUMBER
            AND D.DOCUMENTNUMBER = A.INDEMNITYNUMBER
        ...
        WHERE 
            ...
            AND B.STATUS = 'OPEN'
            AND D.STATUS NOT IN ( 'EXPIRED', 'RETURNED', 'PENDING', 'MARV')
        );
    ...
    INNER JOIN TFSDB2S.PROD_PAYMNT_CICLS_MAPPING F
		ON C.PAYMENTINSTRUMENTTYPE = F.MOP_PRODUCT
    ...
    WHERE
	   ...
	   AND A.TRSTS = 'Y'
	   AND E.STATUS IN ( 'APPROVED', 'POST_APPROVED', 'POSTED');
END P1 @