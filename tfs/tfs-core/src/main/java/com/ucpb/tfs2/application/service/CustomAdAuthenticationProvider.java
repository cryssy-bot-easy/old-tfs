 ...
 * @since 3.1
 */
public final class CustomAdAuthenticationProvider extends AbstractLdapAuthenticationProvider {
	...
	public CustomAdAuthenticationProvider(String domain, String url,
		...
		this.directory = this.directory == null ? null : directoryFromDomain(this.domain);
	}
	...
	public CustomAdAuthenticationProvider(String domain, String url,
			String rootDn, String directory) {
		Assert.isTrue(StringUtils.hasText(url), "Url cannot be empty");
		this.domain = StringUtils.hasText(domain) ? domain.toLowerCase() : null;
		this.url = url;
		this.rootDn = StringUtils.hasText(rootDn) ? rootDn.toLowerCase() : null;
		this.directory = StringUtils.hasText(directory) ? directory.toUpperCase() : null;
	}
	...
	private DirContext bindAsUser(String username, String password) {
		...
		String bindPrincipal = createBindPrincipalByDirectory(username);
		System.out.println("bindPrincipal = " + bindPrincipal);
	...
	private void raiseExceptionForErrorCode(int code, NamingException exception) {
		...
		Throwable cause = new CustomAdAuthenticationException(hexString,
	...
	private String directoryFromDomain(String domain) {
		String[] tokens = StringUtils.tokenizeToStringArray(domain, ".");
		StringBuilder root = new StringBuilder();
	...
	String createBindPrincipalByDirectory(String username) {
		if (directory == null || username.toLowerCase().startsWith(directory)) {
			return username;
		}
